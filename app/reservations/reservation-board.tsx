...
export function ReservationBoard() {
  ...
  const dates = useMemo(() => Array.from({ length: 15 }, (_, index) => { const date = new Date(); date.setDate(date.getDate() + index); return date; }), []);
  ...
  return <section className="section reservation-board-section"><div className="shell">
    ...
    <div className="reservation-notice"><strong>예약 전 확인</strong><div><p>최소 진행 인원은 4명입니다. 1–3명은 오픈룸을 선택하면 다른 팀과 함께 예약할 수 있습니다. 예약 시간 10분 전까지 도착해 주세요.</p><p className="reservation-refund-warning">이용 당일 고객 사유 취소와 무단 불참은 환불되지 않습니다.</p></div></div>
    <div className="date-picker"><div className="filter-label"><span>01</span><div><strong>날짜 선택</strong><small>15일 이내 예약 가능</small></div></div><div className="date-scroll">{dates.map((item, index) => { const value = formatDate(item); return <button className={date === value ? "is-selected" : ""} type="button" key={value} onClick={() => { setLoading(true); setError(""); setDate(value); }}><small>{index === 0 ? "오늘" : item.toLocaleDateString("ko-KR", { weekday: "short" })}</small><strong>{item.getDate()}</strong><span>{item.getMonth() + 1}월</span></button>; })}</div></div>
