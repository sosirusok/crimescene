...
type Result = { reservation: { lookupCode: string; themeTitle: string; playDate: string; startTime: string; partySize: number; totalAmount: number }; payment: { enabled: boolean } };

export function ReservationComplete() {
  const [result, setResult] = useState<Result | null>(null);
  const code = useSearchParams().get("code") ?? "";
  useEffect(() => {
    const stored = sessionStorage.getItem(`reservation:${code}`);
  ...
  return <section className="complete-section"><div className="complete-card">
    <span className="complete-seal">CASE<br/>FILED</span><p className="eyebrow">Reservation received</p><h1>예약이 접수되었습니다.</h1><p className="complete-lead">아래 예약번호를 보관해 주세요. 예약 확인과 취소에 사용할 수 있습니다.</p>
    <div className="lookup-code"><small>예약번호</small><strong>{result?.reservation.lookupCode || code || "확인 중"}</strong></div>
